...
class Wait_utility:
    ...
    def wait_until_cancel_link_is_displayed(self, driver):
        WebDriverWait(driver, self.EXPLICIT_WAIT).until(EC.presence_of_element_located((By.XPATH, "//a[text()='Cancel']")))

    def wait_until_logout_is_displayed(self, driver):
        WebDriverWait(driver, self.EXPLICIT_WAIT).until(EC.presence_of_element_located((By.CSS_SELECTOR, "a i.fa-power-off")))

    def wait_until_text_is_displayed(self, driver, element, text):
        WebDriverWait(driver, self.EXPLICIT_WAIT).until(EC.text_to_be_present_in_element(element, text))
#EC = Expected conditions - inbuild class
#can check web_element conditions like alerts/to be clickable/
